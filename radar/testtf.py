...
async def get_all_matches(timing, headers, url, session, proxy, semaphore):
    async with semaphore:
        ...
        try:
            async with session.post(url, headers=headers, params=querystring_common, proxy=proxy,
            ...
            print(f"Timeout occurred for request with timing: {timing}")
            return []
...
async def get_urls(payload, url, headers, session, proxy, semaphore):
    async with semaphore:
        event_id = payload[0]
        markets = []
        for ci in payload[1:]:
            if ci != 'MATCH':
                ...
                querystring = {"event_id": event_id, "combo": "false", "outright": "false",
                               ...
                               "timezone": "Europe/Moscow"}
            try:
                async with session.post(url, headers=headers, params=querystring, proxy=proxy,
                ...
                print(f"Timeout occurred for request with payload: {payload}")
                return []
    return markets
...
async def tf():
    ...
    vhod = [('inplay', 'now'), ('today', 'now'), ('early', (dt.now() + td(days=1)).strftime("%Y-%m-%d")), ('early', (dt.now() + td(days=2)).strftime("%Y-%m-%d")), ('early', (dt.now() + td(days=3)).strftime("%Y-%m-%d"))]
    # vhod = [('today', 'now')]
    proxies = ['http://45.145.160.130:8000', 'http://45.143.245.98:8000', 'http://185.24.109.64:8000', 'http://38.170.137.236:8000', 'http://45.143.246.99:8000', 'http://45.143.245.225:8000']
    connector = aiohttp.TCPConnector(limit=100)
    semaphore = asyncio.Semaphore(100)
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [asyncio.ensure_future(
            get_all_matches(timing, headers, url, session, choice(proxies), semaphore)) for timing
            in vhod]
        results = []
        ...
        tasks = [asyncio.ensure_future(get_urls(payload, url, headers, session, choice(proxies), semaphore)) for
                 payload in payloads]
        results = []
        for task in asyncio.as_completed(tasks):
            result = await task
