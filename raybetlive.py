    ...
    for item in game_result:
        ...
        if len(kefs) > 0:
            ...
            if time < current_time:
                ...
                if time < current_time:
                    ...
                    bet_list.append(item['result']['team'][0]['team_name'])
                    bet_list.append(item['result']['team'][1]['team_name'])
        ...
        if len(kefs) > 0:
            ...
            if time < current_time:
                ...
                bet_fora.append('raybet')
                game_check = item['result']['game_name']
                if game_check in game_name:
                    game_check = game_name[game_check]
                bet_fora.append(game_check)
                bet_fora.append(item['result']['tournament_short_name'])
                bet_fora.append(item['result']['team'][0]['team_name'])
                bet_fora.append(item['result']['team'][1]['team_name'])
                bet_fora.append(time)
        ...
        if len(kefs) > 4:
            ...
            if len(kefs) > 0:
                ...
                if time < current_time:
                    ...
                    bet_total.append('raybet')
                    game_check = item['result']['game_name']
                    if game_check in game_name:
                        game_check = game_name[game_check]
                    bet_total.append(game_check)
                    bet_total.append(item['result']['tournament_short_name'])
                    bet_total.append(item['result']['team'][0]['team_name'])
                    bet_total.append(item['result']['team'][1]['team_name'])
                    bet_total.append(time)
...
def get_game_result(bet_id):
    ...
    for id in bet_id:
        ...
        response = session.get('https://vnimpvgameinfo.esportsworldlink.com/v2/odds', params=params)
...
def main():
    ...
    for page in range(1, 3):
        ...
        response = session.get('https://vnimpvgameinfo.esportsworldlink.com/v2/match', params=params)
        result_list.append(response.json())

    bet_list_final = get_id(result_list)
    #print(bet_list_final)
